...
exports.cleanShellOutput = cleanShellOutput;
const vscode = __importStar(require("vscode"));
const RECENT_RESULT_TTL_MS = 60000;
...
class TerminalManager {
    constructor(maxBufferLines) {
        this.outputBuffers = new Map();
        this.disposables = [];
        this.ownExecutions = new Set();
    ...
    registerExecution(e, commandLine) {
        let resolveEnd;
    ...
    setupListeners() {
        this.disposables.push(vscode.window.onDidStartTerminalShellExecution(async (e) => {
        ...
        this.disposables.push(vscode.window.onDidEndTerminalShellExecution((e) => {
    ...
    resolveShellProfile(name) {
        const lower = name.toLowerCase();
        const platform = process.platform === 'win32' ? 'windows' : process.platform === 'darwin' ? 'osx' : 'linux';
        const profiles = vscode.workspace.getConfiguration('terminal.integrated.profiles').get(platform);
        if (profiles) {
    ...
    busyError(terminalName, state) {
        ...
        const tailSection = tail.length > 0
            ? `last output:\n  ${tail.join('\n  ')}`
            : 'no output yet';
        return new Error(`Terminal '${terminalName}' is busy running: ${state.commandLine} ` +
    ...
    async executeCommand(command, terminalName, timeoutMs = 300000, onChunk) {
        ...
        if (!terminal.shellIntegration) {
            ...
            return {
                output: '[Shell integration unavailable — command sent via sendText. terminal_wait ' +
        ...
        if (outcome === 'done') {
            const exitCode = await Promise.race([
                state.endPromise,
                new Promise((resolve) => setTimeout(() => resolve(undefined), END_EVENT_WAIT_MS)),
            ]);
    ...
    async startBackgroundExecution(command, terminalName) {
        ...
        if (!terminal.shellIntegration) {
            ...
            return {
                ...
                message: `[Shell integration unavailable — command sent via sendText to '${terminal.name}'. ` +
        ...
        return {
            terminalName: terminal.name,
            message: `Command started in terminal '${terminal.name}': ${command}\n` +
    ...
    async waitForExecution(terminalName, timeoutMs = 300000) {
        ...
        if (state) {
            ...
            const endPromise = state.endPromise.then((exitCode) => ({ kind: 'done', exitCode }));
        ...
        throw new Error(`Terminal '${terminalName}' has no active or recent execution. ` +
            `Start one with terminal_run, or check terminal_list.`);
    }
    async createTerminal(name, cwd, shell) {
    ...
    readOutput(terminalName, lines) {
        ...
        if (!buf || buf.lineCount === 0) {
            return ('[No buffered output yet. Output is captured via shell integration — run a\n' +
    ...
    clearBuffer(terminalName) {
        if (terminalName) {
            ...
            if (t)
                this.outputBuffers.get(t.name)?.clear();
        }
    }
    waitForShellIntegration(terminal, timeoutMs) {
